...
export function GoogleAd({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // 현재 슬롯의 요소들 찾기
      const allSlotElements = document.querySelectorAll(
        `[data-ad-slot="${slot}"]`,
      );
      ...
      allSlotElements.forEach((element, index) => {
        ...
        if (isVisible) {
          ...
          console.log(
            `👁️ 슬롯 ${slot}[${index}] 가시적: ${rect.width}x${rect.height}`,
          );
        } else {
          hiddenElements.push(element);
          console.log(
            `🚫 슬롯 ${slot}[${index}] 숨겨짐: ${rect.width}x${rect.height}`,
          );
        }
      });
      ...
      if (visibleElements.length > 1) {
        console.warn(
          `⚠️ 슬롯 ${slot} 중복 ${visibleElements.length}개, 정리 필요`,
        );
